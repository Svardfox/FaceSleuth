...
class GAM_Attention(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, groups=1,
                    base_width=64, dilation=1, norm_layer=None, rate=4):
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(out_channels, int(out_channels / rate), kernel_size=7, padding=3),
            nn.BatchNorm2d(int(out_channels / rate)),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(out_channels / rate), out_channels, kernel_size=7, padding=3),
        ...
        self.conv3 = conv1x1(out_channels,1)
    ...
    def forward(self, x):
        ...
        attn = attn.repeat(1, self.out_channels, 1, 1) 
        if if_attn:
            out = identity*attn
