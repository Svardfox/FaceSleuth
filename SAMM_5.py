...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str,
                        default='F:/dataset/SAMM', help='Raf-DB dataset path.')
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, num_loso, transform=None, basic_aug=False, transform_norm=None):
        ...
        df = pd.read_excel(os.path.join(
            self.raf_path, 'SAMM_Micro_FACS_Codes_v2.xlsx'), usecols=[0, 1, 3, 4, 5, 8, 9])
        ...
        for (f, sub, onset, apex, offset, label_all, label_au) in zip(File_names, Subject, Onset_num, Apex_num,
            ...
            if label_all == 'Happiness' or label_all == 'Anger' or label_all == 'Contempt' or label_all == 'Other' \
                     or label_all == 'Surprise':
    ...
    def __getitem__(self, idx):
        ...
        path_on0 = GetSAMM(self.raf_path, f, on0, sub)
        ''' path_on1 = GetSAMM(self.raf_path, f, on1, sub)
        path_on2 = GetSAMM(self.raf_path, f, on2, sub)
        path_on3 = GetSAMM(self.raf_path, f, on3, sub) '''
        path_apex0 = GetSAMM(self.raf_path, f, apex0, sub)
        ''' path_off0 = GetSAMM(self.raf_path, f, off0, sub)
        path_off1 = GetSAMM(self.raf_path, f, off1, sub)
        path_off2 = GetSAMM(self.raf_path, f, off2, sub)
        path_off3 = GetSAMM(self.raf_path, f, off3, sub) '''
...
def GetSAMM(raf_path, filename, picNum, sub):
    """
    SAMM图片命名帧数的位数并不同一,通过GetSAMM方法返回正确图片地址
    """

    picName3 = filename[0:4] + picNum.zfill(3) + '.jpg'
    picName4 = filename[0:4] + picNum.zfill(4) + '.jpg'
    picName5 = filename[0:4] + picNum.zfill(5) + '.jpg'
    path3 = os.path.join(raf_path, sub, filename, picName3)
    path4 = os.path.join(raf_path, sub, filename, picName4)
    path5 = os.path.join(raf_path, sub, filename, picName5)
...
class MMNet(nn.Module):
    ...
    def forward(self, x1, x5):
        ...
        POS = self.vit_pos(self.resize(x1)).transpose(
            1, 2).view(B, 512, 14, 14)  # [B,512,14,14]

        act = x5 - x1
        act = self.conv_act(act)
        # main branch and fusion
        out, _ = self.main_branch(act, POS)  # [B,180,112,112],[B,512,14,14]
...
def run_training():
    ...
    LOSO = [ '16', '9', '30', '13', '23', '11', '12', '14', '28', '19', '26', '18', '10',
            '20', '21', '22', '15', '6', '25', '7', '31', '32', '33', '34', '35', '37','17']
    ...
    for subj in LOSO:
        ...
        if args.optimizer == 'adam':
            optimizer_all = torch.optim.AdamW(
                params_all, lr=0.0008, weight_decay=0.6)
            # optimizer for MMNet
        ...
        for epoch in range(1, 75):
            ...
            sinceT = time.time()
            for batch_i, (
                    image_on0, image_apex0, label_all,
                ...
                starter.record()
                # train MMNet
                Outputs = net_all(image_on0,image_apex0)

                loss_all = criterion(Outputs, label_all)
                ...
                correct_num = torch.eq(predicts, label_all).sum().item()
                #print('[batch_(%d)] correct_num: %d' %(batch_i,correct_num))
                correct_sum += correct_num

            # lr decay
            if epoch <= 75:
                scheduler_all.step() #前50个epoch，lr = lr*0.987
            if epoch >= 0:
                acc = correct_sum / float(train_dataset.__len__())
                ...
                print('[Epoch %d] Training accuracy: %.4f. Loss: %.3f Inference time: %.4f(py),Data time:%.4f Total time: %.4f' % 
                      (epoch, acc, running_loss,curr_time/1000,data_time,total_time))
            ...
            with torch.no_grad():
                ...
                for batch_i, (
                    ...
                    loss = criterion(Outputs, label_all)
                    running_loss += loss
                    iter_cnt += 1
                    predicts = torch.max(Outputs, dim=1)[1]
                    correct_num = torch.eq(predicts, label_all).sum().item()
                    ...
                    for cls in range(5):
                        ...
                        for element in predicts:
                            if element == cls:
                                pos_pred[cls] = pos_pred[cls] + 1 #存储预测各类型数量数组，TP+FP
                        for element in label_all:
                            if element == cls:
                                pos_label[cls] = pos_label[cls] + 1  #存储实际各类型数量数组TP+FN
                        for elementp, elementl in zip(predicts, label_all):
                            if elementp == elementl and elementp == cls:
                                TP_c[cls] = TP_c[cls] + 1               #True Positive存储各类型正确数量
                        for elementp,elementl in zip(predicts,label_all):
                            if elementp != elementl and elementp == cls:
                                FP_c[cls] = FP_c[cls] + 1               #False Positive
                        FN_c[cls] = pos_label[cls] - TP_c[cls]
                    ...
                    for cls in range(5):
                        ...
                        """
                        if pos_label[cls] != 0 or pos_pred[cls] != 0:
                            count += 1
                            F1_c[cls] = 2*TP_c[cls]/(2*TP_c[cls] + FP_c[cls] + FN_c[cls])

                    
                    UF1 = F1_c.sum()/count                              #Unweighted F1-score
                ...
                print("[Epoch %d] Validation accuracy:%.4f. Loss:%.3f, UF1-score:%.3f" %
                      (epoch, acc, running_loss, UF1))
