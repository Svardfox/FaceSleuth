...
from CA_block import resnet18_pos_attention, OrientationPoolingVec
...
import json
import matplotlib
matplotlib.use('Agg')  # 服务器环境无显示器时使用非交互后端
...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='../CASMEII', help='Raf-DB dataset path.')
...
def run_training():
    ...
    for subj in LOSO:
        ...
        theta_params = []
        other_params = []
        for name, param in net_all.named_parameters():
            if name.endswith('pool.vec'):
        ...
        if args.optimizer == 'adam':
            ...
            optimizer_all = torch.optim.AdamW([
                {'params': other_params, 'lr': 0.0008, 'weight_decay': 0.6},
                {'params': theta_params, 'lr': 0.005, 'weight_decay': 0.2}
            ])
            ...
            optimizer_all = torch.optim.SGD([
                {'params': other_params, 'lr': args.lr, 'momentum': args.momentum, 'weight_decay': 1e-4},
                {'params': theta_params, 'lr': 0.01, 'momentum': args.momentum, 'weight_decay': 0.0}
            ])
        else:
        ...
        for i in range(1, 100):
            ...
            sinceT = time.time()
            # 使用tqdm创建进度条
            pbar = tqdm(train_loader, desc=f'Epoch {i}', leave=True)
            # 初始化theta_grad_accum
            theta_grad_accum = {}
            
            for batch_i, (image_on0, image_apex0, label_all, label_au) in enumerate(pbar):
                ...
                # 记录本 batch 中 θ 的梯度
                for name, module in net_all.main_branch.named_modules():
                    if isinstance(module, OrientationPoolingVec) and module.vec.grad is not None:
                        grad_val = module.vec.grad.norm().item()  # 取L2范数作为梯度大小（标量）
                        theta_grad_accum[name] = theta_grad_accum.get(name, 0.0) + grad_val
                ...
                # 更新进度条信息
                pbar.set_postfix({
                    'loss': f'{loss_all.item():.3f}',
                    'acc': f'{correct_num.float()/batch_sz:.3f}',
                    'time': f'{starter.elapsed_time(ender)/1000:.2f}s'
                })
            ...
            if i >= 0:
                ...
                # 打印训练信息
                print(f"\n{'='*50}")
                print(f"训练信息 - Epoch {i} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"{'='*50}")
                print(f"训练集:")
                print(f"  准确率: {acc:.4f}")
                print(f"  损失值: {running_loss:.3f}")
                print(f"  推理时间: {curr_time/1000:.2f}s")
                print(f"  数据处理时间: {data_time:.2f}s")
                print(f"  总时间: {total_time:.2f}s")
                
                # 打印θ参数信息
                print(f"\nθ参数信息:")
                for name, module in net_all.main_branch.named_modules():
                    if isinstance(module, OrientationPoolingVec):
                        theta_value = module.angle().item()
                        print(f"  {name}: {theta_value*180/math.pi:.2f}°")
                        theta_dict = {}
                        theta_dict[name] = theta_value
                        theta_log[name] = theta_log.get(name, [])
                        theta_log[name].append(theta_value * 180 / math.pi)
                
                # 打印θ梯度信息
                print(f"\nθ梯度信息:")
                for name, grad_sum in theta_grad_accum.items():
                    grad_avg = grad_sum / iter_cnt if iter_cnt else 0.0
                    print(f"  {name}: {grad_avg:.6e}")
                    grad_log[name] = grad_log.get(name, [])
                    grad_log[name].append(grad_avg)
        ...
        save_path = f'./training_record/2/theta_LOSO_{subj}.json'
        with open(save_path, 'w') as f:
            json.dump(theta_dict, f, indent=2)
        print(f"SOA θ参数已保存到 {save_path}")

        # === 绘制并保存 θ 随 epoch 变化曲线 ===
        for name, series in theta_log.items():
            plt.figure()
            plt.plot(series, label=name)
            plt.xlabel('Epoch')
            plt.ylabel('θ (deg)')
            plt.title(f'{name} θ 曲线 (Sub {subj})')
            plt.legend()
            plt.tight_layout()
            fig_path = f'./training_record/{subj}_{name.replace(".","_")}_theta_curve.png'
            plt.savefig(fig_path)
            plt.close()
            print(f'θ 曲线已保存至 {fig_path}')
    return record
...
if __name__ == "__main__":
    ...
    record.to_excel('./training_record/2' + ' vec'+'.xlsx')
